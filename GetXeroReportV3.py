...
# -----------------------------
# Safe request function with retries, auto token refresh
# -----------------------------
...
# -----------------------------
# Export invoices (Sales or Expenses)
# -----------------------------
...
def export_all_invoices_combined(tenant_id, filename="all_invoices.txt"):
    invoice_types = [("ACCREC", "Sale"), ("ACCPAY", "Expense")]
    rows = []

    for inv_type, label in invoice_types:
        page = 1
        while True:
            url = f"https://api.xero.com/api.xro/2.0/Invoices?page={page}&where=Type==\"{inv_type}\""
            data = safe_request(url, tenant_id, page)
            if not data:
                break
            invoices = data.get("Invoices", [])
            ...
            print(f"Fetched {label} page {page}")
            page += 1
            time.sleep(0.5)

    # Write combined file
    with open(filename, "w", encoding="utf-8") as f:
        header = ["Type","InvoiceNumber","Date","Contact","Description","AccountCode","Quantity","UnitAmount","LineAmount","InvoiceTotal","Status"]
        f.write("|".join(header) + "\n")
        for r in rows:
            r = [str(x) if x else "" for x in r]
            f.write("|".join(r) + "\n")

    print(f"All sales and expenses exported to {filename}")
...
def main():
    ...
    export_all_invoices_combined(tenant_id, "JOP_all_invoices.txt")
