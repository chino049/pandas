...
# -----------------------------
# Paid expense claims (with progress printing)
# -----------------------------
def export_paid_expense_claims_by_date(
    tenant_id,
    ...
    where_clause = (
        f'Status=="PAID" && '
        f'Date>=DateTime({sy},{sm},{sd}) && '
        f'Date<=DateTime({ey},{em},{ed})'
    )

    list_url = f"https://api.xero.com/api.xro/2.0/ExpenseClaims?where={where_clause}"

    # Single request for all paid claims
    data = safe_request(list_url, tenant_id)
    if not data:
        print("⚠️ Failed to fetch expense claims")
        return []

    claims = data.get("ExpenseClaims", [])
    print(f"Found {len(claims)} paid expense claims")

    # Export claim-level data only
    for claim in claims:
        rows.append([
            claim.get("ExpenseClaimID"),
            format_xero_date(claim.get("Date")),
            claim.get("Contact", {}).get("Name", ""),
            "",          # Description
            "",          # AccountCode
            "",          # Quantity
            "",          # UnitAmount
    ...
    print(f"Exported {len(rows)} expense claim rows to {filename}")
    return rows
...
#     # ---- Manual retry loop for the list request ----
#     for attempt in range(5):
#         data = safe_request(list_url, tenant_id)
#         if data is not None:
#             break
#         wait = 2 * (attempt + 1)
#         print(f"Rate limit or error fetching list, sleeping {wait}s before retry...")
#         time.sleep(wait)
#     else:
#         print("⚠️ Failed to fetch the list of paid expense claims after 5 attempts")
#         return []
...
#     # ---- Process each claim ----
#     for claim in claims:
#         claim_id = claim.get("ExpenseClaimID")
#         contact_name = claim.get("Contact", {}).get("Name", "")
#         date = format_xero_date(claim.get("Date"))
#         total = claim.get("Total")
#         status = claim.get("Status")

#         detail_url = f"https://api.xero.com/api.xro/2.0/ExpenseClaims/{claim_id}"

#         # Manual retry for claim detail
#         for attempt in range(5):
#             detail_data = safe_request(detail_url, tenant_id)
#             if detail_data is not None:
#                 break
#             wait = 2 * (attempt + 1)
#             print(f"Rate limit or error fetching claim {claim_id}, sleeping {wait}s before retry...")
#             time.sleep(wait)
#         else:
#             print(f"⚠️ Failed to fetch details for claim {claim_id}, exporting minimal row")
#             rows.append([
#                 claim_id,
#                 date,
#                 contact_name,
...
#         detail = detail_data.get("ExpenseClaims", [{}])[0]
...
#                     date,
#                     contact_name,
#                     receipt.get("LineItem_Description", ""),
#                     receipt.get("LineItem_AccountCode", ""),
#                     receipt.get("LineItem_Quantity", ""),
#                     receipt.get("LineItem_UnitAmount", ""),
#                     receipt.get("LineItem_LineAmount", ""),
#                     total,
#                     status,
#                     "ExpenseClaim"
#                 ])
...
# -----------------------------
# NEW: Export ALL sales invoices (including draft, paid, voided, deleted)
# -----------------------------
def export_all_sales_invoices(
    tenant_id,
    start_date="2010-01-01",
    end_date="2030-12-31",
    filename="JOP_all_sales_invoices.txt"
):
    page = 1
    rows = []
    total_count = 0
    while True:
        where = (
            f'Type=="ACCREC" && '
            f'(Status=="DRAFT" || Status=="SUBMITTED" || Status=="AUTHORISED" || '
            f'Status=="PAID" || Status=="VOIDED" || Status=="DELETED") && '
            f'Date>=DateTime({start_date}) && Date<=DateTime({end_date})'
        )
        url = f"https://api.xero.com/api.xro/2.0/Invoices?page={page}&pageSize=1000&where={where}"
        data = safe_request(url, tenant_id, page)
        if not data:
            break
        invoices = data.get("Invoices", [])
        ...
        total_count += len(invoices)
        print(f"Fetched all sales page {page} ({len(invoices)} invoices)")
        page += 1
        time.sleep(0.6)
    with open(filename, "w", encoding="utf-8") as f:
        header = ["InvoiceNumber","Date","Contact","Description","AccountCode","Quantity","UnitAmount","LineAmount","Total","Status","Type"]
        ...
        for r in rows:
            f.write("|".join([str(x) if x is not None else "" for x in r]) + "\n")
    print(f"All sales invoices exported to {filename} ({total_count} total)")
    return rows

# -----------------------------
# NEW: Export ALL expenses (including legacy reimbursements, voided, deleted)
# -----------------------------
def export_all_expenses(
    tenant_id,
    start_date="2010-01-01",
    end_date="2030-12-31",
    filename="JOP_all_expenses.txt"
):
    page = 1
    rows = []
    total_count = 0
    while True:
        ...
        total_count += len(invoices)
        print(f"Fetched all expenses page {page} ({len(invoices)} records)")
        page += 1
        time.sleep(0.6)
    with open(filename, "w", encoding="utf-8") as f:
        header = ["InvoiceNumber","Date","Contact","Description","AccountCode","Quantity","UnitAmount","LineAmount","Total","Status","Type"]
        ...
        for r in rows:
            f.write("|".join([str(x) if x is not None else "" for x in r]) + "\n")
    print(f"All expenses exported to {filename} ({total_count} total)")
    return rows
...
def export_all_paid_invoices_combined(
    ...
    print(f"Total sales invoice rows exported: {len(sales)}\n")
    ...
    print(f"Total expense invoice rows exported: {len(expenses)}\n")
    ...
    print(f"Total expense claim rows exported: {len(expense_claims)}\n")

    # Combine everything (NO duplicate filtering)
    all_rows = sales + expenses + expense_claims

    print(f"Writing combined file with {len(all_rows)} total rows...")
    ...
    print(
        f"All paid sales, supplier invoices, and expense claims "
        f"exported to {filename} ({len(all_rows)} rows)"
    )
...
# -----------------------------
# NEW: Run and export a Profit & Loss report (or adapt for other reports)
# -----------------------------
def export_profit_and_loss_report(
    tenant_id,
    from_date="2025-01-01", #None,           # e.g. "2025-01-01"
    to_date="2025-12-31",   #None,             # e.g. "2025-12-31"
    filename="JOP_profit_and_loss.json"
):
    """
    Fetches the ProfitAndLoss report via Xero API and saves the full JSON.
    Optional: add periods=3&timeframe=MONTH, trackingCategoryID=..., etc.
    """
    base_url = "https://api.xero.com/api.xro/2.0/Reports/ProfitAndLoss"
    #base_url = "https://api.xero.com/api.xro/2.0/Reports/BankTransactions"
    #base_url = "https://api.xero.com/api.xro/2.0/BankTransactions"
    #base_url = "https://api.xero.com/api.xro/2.0/Reports/Bank%20transactions%20by%20date%20JOP%20All"
    params = {}
    if from_date:
    ...
    # Optional extras you can uncomment/add:
    # params["periods"] = "3"
    # params["timeframe"] = "MONTH"
    # params["standardLayout"] = "true"
    # params["paymentsOnly"] = "true"
    # params["trackingCategoryID"] = "your-tracking-guid-here"

    query_string = "&".join([f"{k}={v}" for k,v in params.items()])
    url = base_url + ("?" + query_string if query_string else "")

    print(f"Fetching Profit & Loss report from {from_date or 'default'} to {to_date or 'default'}...")
    data = safe_request(url, tenant_id, method="GET")
    
    if not data or "Reports" not in data or not data["Reports"]:
        print("Failed to retrieve Profit & Loss report or empty response.")
        return None

    report = data["Reports"][0]  # usually only one report returned
    print(f"Report retrieved: {report.get('ReportName')} as at {report.get('ReportDate')}")

    # Save full structured JSON
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4)
    print(f"Full Profit & Loss report saved to {filename}")

    # Optional: print a simple summary of key totals to console
    print("\nQuick summary (top-level totals):")
    for row in report.get("Rows", []):
        if row.get("RowType") in ("SummaryRow", "Row") and row.get("Cells"):
            title = row.get("Title", "Unnamed")
            values = [cell.get("Value", "") for cell in row["Cells"]]
            print(f"{title:<30} | {' | '.join(values[:3])} ...")  # show first few columns
...
def main():
    ...
    # Original exports (commented out as in your script)
    ...
    # NEW: Export ALL sales invoices (including archived/voided/deleted)
    # print("\nExporting ALL sales invoices (including draft, paid, voided, deleted)...")
    # export_all_sales_invoices(tenant_id)

    # # NEW: Export ALL expenses (including legacy reimbursements/old claims)
    # print("\nExporting ALL expenses (including archived/old claims via ACCPAY)...")
    # export_all_expenses(tenant_id)

    # NEW: Run a Profit & Loss report (customize dates below)
    #print("\nRunning Profit & Loss report...")
    # Example: full year 2025 — change as needed
    # export_profit_and_loss_report(
    #     tenant_id,
    #     from_date="2025-01-01",
    #     to_date="2025-12-31",
    #     # filename="JOP_profit_and_loss_2025.json"  # optional override
    # )
