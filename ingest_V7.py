...
from cleanCSVXero_V1 import cleanXeroCSVfile as cxf
...
def run(app, csv_file, dry_run):
    ...
    if app == 'xero':
        ...
        # #     cur = conn.cursor()
        #     cur.execute("select * from "+xeroDB)
        #     # test table
        #     #cur.execute("select * from test_xeroDB")
        #     pgl = cur.fetchall()
        #     #print(type(pgl), pgl)
        #
        # except Exception as pp:
        #     print("ERROR: 74 Cannot reach/read the database or table", pp)
        #     sys.exit(93)
        # #
        # cur.close()
        # conn.close()
        ...
        #         #print(re)
        #         # re[0] = re[0].replace('\ufeff', '')
        #         re[2] = re[2].strftime("%d %b %Y") #(datetime.datetime('%Y-%m-%d'))
        #         re[8] = "{:.2f}".format(re[8])
        #         re[9] = "{:.2f}".format(re[9])
        #         re[12] = "{:.2f}".format(re[12])
        #         re[15] = "{:.2f}".format(re[15])
        #         re[16] = "{:.2f}".format(re[16])
        #         # re[4] = re[4].replace("\r", "").replace("\n", "")
        #         # if re[5] == None:
        #         #     re[5] = ""
        #
        #
        #         pclean=[re[0], re[1], re[2], re[3], re[4], re[5], re[6], re[7], re[8], re[9], re[10], re[11], re[12], re[13], re[14], re[15], re[16], re[17] ]
        #         #print("..pclean", pclean)
        #         # ..pclean ['200', 'Sales', datetime.date(2019, 2, 1), 'Revenue', 'Unit F', 'Unit F - Electricity Unit F', 'INV-0971', 'Electricity', 2024.0, 0.0, 'Receivable Invoice', '610 - Accounts Receivable', 2024.0, '0.00%', '', 0.0, 2024.0, 'Tax on Sales']
        #         #..pclean ['473', 'Materials, Repairs and Maintenance', '21 Sep 2016', 'Expense', '', 'Jasmin D - materials ceiling unit c(corniza 1x2x12,nail 1"2\',Qde blade p thinner', '', '', 603.0, 0.0, 'Expense Claim', '801 - Unpaid Expense Claims', 603.0, '0.00%', '', 603.0, 0.0, 'Tax on Purchases']
        #         #..       ('473', 'Materials, Repairs and Maintenance', '21 Sep 2016', 'Expense', '', 'Jasmin D - materials ceiling unit c(corniza 1x2x12,nail 1"2\',Qde blade p thinner', '', '', '603.00', '0.00', 'Expense Claim', '801 - Unpaid Expense Claims', '603.00', '0.00%', '', '603.00', '0.00', 'Tax on Purchases')
        #
        ...
        #     print("ERROR 23 Could not clean PG records", sw)
        #     sys.exit(61)
...
# CREATE table bank_transactions_102124 as table bank_transactions;
                    ...
                    try:
                        ...
                        sys.exit(38)
            if not dry_run:
                print("Renaming file ", csv_file, "to ", csv_file+"processed")
                os.rename(csv_file, csv_file+"processed")
