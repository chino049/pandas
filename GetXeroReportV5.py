...
# -----------------------------
# Convert Xero date to readable format
# -----------------------------
def xero_date_to_string(xdate):
    try:
        millis = int(xdate.split("(")[1].split("+")[0])
        ...
        for inv in invoices:
            ...
            for line in inv.get("LineItems", []):
                rows.append([
                    ...
                    xero_date_to_string(inv.get("Date")),
                    contact,
...
# -----------------------------
# Export paid invoices only (Sales/Expenses)
# -----------------------------
def export_paid_invoices(tenant_id, invoice_type, filename, type_label):
    ...
    while True:
        ...
        if not invoices:
            break
        for inv in invoices:
            # Skip non-paid or voided invoices
...
def main():
    ...
    # -----------------------------
    # Original functionality (all exports)
    # -----------------------------
